"""Collect v3 (leak-fixed, outcome pretext OFF, ID-free, budget-corpus, max_trace_len=64) label-efficiency runs into one CSV on stdout.
...
role_corpus=budget and a v3 backbone (or the random_role sentinel) are kept; the latest completed run wins per
(log, arm, seed)."""
...
for d in sorted(glob.glob("outputs/label_efficiency/*/")):
    ...
    ok = all(("-v3-" in str(v)) or (v == "random_role") for v in bbs.values())
