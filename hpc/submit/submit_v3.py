...
Backbone ids are resolved from outputs/backbones/*/manifest.json by tag (-v3-<variant>), newest completed with backbone.pt."""
...
def backbone(variant):
    ...
    for f in glob.glob("outputs/backbones/*/manifest.json"):
        ...
        tagged = str(c.get("tag", "")) == "-v3-" + variant or str(c.get("name", "")).endswith("-v3-" + variant) or os.path.basename(os.path.dirname(f)).split("-v3-")[-1].rsplit("-", 1)[0] == variant
    ...
    for log, (path, extra) in LOGS.items():
        ...
        for arm, ov in ARMS.items():
            for sd in (0, 1, 2):
                ...
                jid = sbatch("r20-main-%s-%s-s%d" % (log, arm, sd), tl, args)
                for c in range(2 if log == "BPI17" else 1):
                    jid = sbatch("r20-main-%s-%s-s%d-c%d" % (log, arm, sd, c + 1), tl, args, dep=jid)
    ...
    for v in ("mlp15", "raw15", "gin11", "gin0", "latent0", "norole"):
        ...
        for log, (path, extra) in LOGS.items():
            ...
            for sd in (0, 1, 2):
                ...
                jid = sbatch("r20-abl-%s-%s-s%d" % (v, log, sd), TIME["ablation"]["BPI17"] if log == "BPI17" else TIME["ablation"]["_"], args)
                jid = sbatch("r20-abl-%s-%s-s%d-c1" % (v, log, sd), TIME["ablation"]["BPI17"] if log == "BPI17" else TIME["ablation"]["_"], args, dep=jid)
