...
base = ["task=evaluate", "evaluate=label_efficiency", "~evaluate.backbones.ar", "~evaluate.backbones.random",
        "evaluate.eval_dataset=BPI20ID", "evaluate.eval_log.path=/workspace/data/raw/BPI20ID.xes", "model=role_gin15",
        "+evaluate.backbones.gin15allw1=backbone-20260825-125337-multi-none-frozen-gin15allw1-b1041f",
        "+evaluate.backbones.random_role=random_role",
...
for sd in (0, 1, 2):
    args = " ".join(base + ["evaluate.seeds=[%d]" % sd]); name = "r8-randrole-BPI20ID-s%d" % sd
    cmd = ["sbatch", "--job-name=" + name, "--time=03:00:00", "--cpus-per-task=4", "--gres=gpu:1", "--export=ALL", "slurm/ncps/run.sbatch"]
