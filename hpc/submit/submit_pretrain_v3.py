...
for v, (cfg, arch, jepa, extra) in VARIANTS.items():
    ...
    args = "model=%s %s %s ar.jepa_weight=%s tag=-v3-%s %s" % (cfg, DATA, COMMON, jepa, v, extra)
    if role: args += " role_init_from=%s" % role
    r = subprocess.run(["sbatch", "--job-name=r20-bb-" + v, "--time=04:00:00", "--cpus-per-task=4", "--gres=gpu:1", "--export=ALL", "slurm/ncps/run.sbatch"],
