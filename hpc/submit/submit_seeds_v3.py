...
COMMON = "freeze_role=true ar.time_weight=1.0 ar.remaining_time_weight=1.0 model.id_dropout=1.0 ar.outcome_weight=0 ar.role_contrast_weight=0"
def role_run():
    hits = []
...
for v, jepa in (("gin15", "1.0"), ("latent0", "0")):
    for seed in (1, 2):
        args = "model=role_gin15 %s %s ar.jepa_weight=%s tag=-v3-%s-s%d seed=%d role_init_from=%s" % (DATA, COMMON, jepa, v, seed, seed, role)
        r = subprocess.run(["sbatch", "--job-name=r20-bb-%s-s%d" % (v, seed), "--time=06:00:00", "--cpus-per-task=4", "--gres=gpu:1", "--export=ALL", "slurm/ncps/run.sbatch"],
