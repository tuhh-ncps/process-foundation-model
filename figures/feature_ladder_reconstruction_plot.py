...
def make_combined(tasks: list[tuple[str, str, str, str, float, list[float], float]], name: str) -> str:
    ...
    fig, ax = plt.subplots(figsize=(6.89, 5.65))   # height set so w/h matches the descriptor heatmap (1.22), text scaled to read like its 15 pt
    ...
    for i, (task, colour, marker, short, scale, ticks, head) in enumerate(tasks):
        ...
        tx = ax.twinx()
        if i:
            tx.spines["right"].set_position(("axes", 1.0 + 0.125 * i))
        ...
        tx.text(1.0 + 0.125 * i + 0.05, -0.115, f"({i + 1})", transform=ax.transAxes, color=colour,
    ...
    leg1 = tx.legend(handles[:2], labels[:2], loc="upper left", bbox_to_anchor=(0.0, 0.86), **style)
    leg1.set_zorder(20)
