...
def make_combined(tasks: list[tuple[str, str, str, str, float, list[float]]], name: str) -> str:
    ...
    fig, ax = plt.subplots(figsize=(7.70, 5.70))   # PDF 7.20 x 5.24 in: same height as before (text ~13 pt beside the heatmap), wider to fill its subfigure slot
    ...
    for i, (task, colour, marker, short, scale, ticks) in enumerate(tasks):
        ...
        tx = ax.twinx()
        if i:
            tx.spines["right"].set_position(("axes", 1.0 + 0.13 * i))
        ...
        tx.text(1.0 + 0.13 * i, 1.02, f"({i + 1})", transform=ax.transAxes, color=colour, ha="center",
